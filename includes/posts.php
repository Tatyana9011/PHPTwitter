							...
							<div class="tweet__wrapper">
								<header class="tweet__header">
									<h3 class="tweet-author"><?php echo $post["name"];?>
										<a href="<?php echo get_url('user_posts.php?id=' . $post["user_id"]); ?>" 
									...
									<?php if(logged_in() && $post['user_id'] == $_SESSION['user']['id']){?>
										<a href="<?php echo get_url('includes/delete_post.php?id=' . $post["id"]); ?>" 
										class="tweet__delete-button chest-icon"></a>
									<?php } ?>
								</header>
						...
						<footer>
							...
							if(logged_in()){
									if(is_post_liked($post['id'])) { ?>
										<a href="<?php echo get_url('includes/delete_like.php?id=' . $post["id"]); ?>"
										class="tweet__like tweet__like_active"><?php echo $likes_count;?></a>
									<?php } else { ?>
										<a href="<?php echo get_url('includes/add_like.php?id=' . $post["id"]); ?>"
										class="tweet__like "><?php echo $likes_count; ?></a>
									<?php }
							} else { ?>
